...
import type { DesignScheme } from '~/types/design-scheme';
import { defaultDesignScheme, designFeatures, designFonts, paletteRoles } from '~/types/design-scheme';
import { applyDesignScheme, applyTheme, loadPersistedDesignScheme } from '~/lib/stores/theme'; // updated import path
...
export const ColorSchemeDialog: React.FC<ColorSchemeDialogProps> = ({ setDesignScheme, designScheme }) => {
  ...
  // local theme selection (light | dark | neon)
  const [localTheme, setLocalTheme] = useState<'light' | 'dark' | 'neon'>('light');
  const currentTheme = useStore(themeStore);

  useEffect(() => {
    // when dialog opens, sync with current persisted design scheme if any
    if (designScheme) {
      setPalette(() => ({ ...defaultDesignScheme.palette, ...designScheme.palette }));
      setFeatures(designScheme.features || defaultDesignScheme.features);
      setFont(designScheme.font || defaultDesignScheme.font);
    } else {
      // try load persisted design scheme
      const persisted = loadPersistedDesignScheme();
      if (persisted) {
        setPalette(() => ({ ...defaultDesignScheme.palette, ...persisted.palette }));
        setFeatures(persisted.features || defaultDesignScheme.features);
        setFont(persisted.font || defaultDesignScheme.font);
      } else {
        setPalette(defaultDesignScheme.palette);
        setFeatures(defaultDesignScheme.features);
        setFont(defaultDesignScheme.font);
    ...
    // sync localTheme with global theme store
    setLocalTheme(currentTheme as 'light' | 'dark' | 'neon');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [designScheme, isDialogOpen]);
  ...
  const handleSave = () => {
    ...
    // 1) apply to UI variables right away
    applyDesignScheme(scheme);

    // 2) persist by calling parent setter if provided
    setDesignScheme?.(scheme);

    // 3) set chosen theme (light/dark/neon) on html
  ...
  const renderColorSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-bolt-elements-textPrimary flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-bolt-elements-item-contentAccent" />
          Color Palette
        </h3>
        <button
          onClick={handleReset}
          ...
          return (
            <div key={f.key} className="feature-card-container p-2">
              <button
                type="button"
                onClick={() => handleFeatureToggle(f.key)}
                className={`group relative w-full p-6 text-sm font-medium transition-all duration-200 bg-bolt-elements-bg-depth-3 text-bolt-elements-item-textSecondary ${f.key === 'rounded'
                  ? isSelected
                    ? 'rounded-3xl'
                    : 'rounded-xl'
                  : f.key === 'border'
  ...
  return (
    <div>
      <IconButton title="Design Palette" className="transition-all" onClick={() => setIsDialogOpen(!isDialogOpen)}>
        <div className="i-ph:palette text-xl" />
      </IconButton>
        ...
        <Dialog>
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              ...
              <DialogDescription className="text-bolt-elements-textSecondary leading-relaxed">
                Customize your color palette, typography, and design features. These preferences will guide the AI in
                creating designs that match your style <strong>and</strong> update the app UI in real-time.
              </DialogDescription>
            </div>

            {/* Theme selector: Light / Dark / Neon */}
            <div className="flex gap-2 items-center">
              <div className="text-sm text-bolt-elements-textSecondary mr-3">App Theme:</div>
              {['light', 'dark', 'neon'].map((t) => (
                <button
                  key={t}
                  type="button"
                  onClick={() => setLocalTheme(t as 'light' | 'dark' | 'neon')}
                  className={`px-3 py-2 rounded-md border transition-all duration-150 ${localTheme === t ? 'bg-bolt-elements-item-backgroundAccent text-bolt-elements-item-contentAccent' : 'bg-bolt-elements-bg-depth-1 text-bolt-elements-textSecondary'
                    }`}
                >
